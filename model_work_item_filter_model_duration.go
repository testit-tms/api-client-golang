/*
API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v2.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tmsclient

import (
	"encoding/json"
)

// checks if the WorkItemFilterModelDuration type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WorkItemFilterModelDuration{}

// WorkItemFilterModelDuration Specifies a work item duration range to search for
type WorkItemFilterModelDuration struct {
	From NullableInt32 `json:"from,omitempty"`
	To NullableInt32 `json:"to,omitempty"`
}

// NewWorkItemFilterModelDuration instantiates a new WorkItemFilterModelDuration object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWorkItemFilterModelDuration() *WorkItemFilterModelDuration {
	this := WorkItemFilterModelDuration{}
	return &this
}

// NewWorkItemFilterModelDurationWithDefaults instantiates a new WorkItemFilterModelDuration object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWorkItemFilterModelDurationWithDefaults() *WorkItemFilterModelDuration {
	this := WorkItemFilterModelDuration{}
	return &this
}

// GetFrom returns the From field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WorkItemFilterModelDuration) GetFrom() int32 {
	if o == nil || IsNil(o.From.Get()) {
		var ret int32
		return ret
	}
	return *o.From.Get()
}

// GetFromOk returns a tuple with the From field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WorkItemFilterModelDuration) GetFromOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.From.Get(), o.From.IsSet()
}

// HasFrom returns a boolean if a field has been set.
func (o *WorkItemFilterModelDuration) HasFrom() bool {
	if o != nil && o.From.IsSet() {
		return true
	}

	return false
}

// SetFrom gets a reference to the given NullableInt32 and assigns it to the From field.
func (o *WorkItemFilterModelDuration) SetFrom(v int32) {
	o.From.Set(&v)
}
// SetFromNil sets the value for From to be an explicit nil
func (o *WorkItemFilterModelDuration) SetFromNil() {
	o.From.Set(nil)
}

// UnsetFrom ensures that no value is present for From, not even an explicit nil
func (o *WorkItemFilterModelDuration) UnsetFrom() {
	o.From.Unset()
}

// GetTo returns the To field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WorkItemFilterModelDuration) GetTo() int32 {
	if o == nil || IsNil(o.To.Get()) {
		var ret int32
		return ret
	}
	return *o.To.Get()
}

// GetToOk returns a tuple with the To field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WorkItemFilterModelDuration) GetToOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.To.Get(), o.To.IsSet()
}

// HasTo returns a boolean if a field has been set.
func (o *WorkItemFilterModelDuration) HasTo() bool {
	if o != nil && o.To.IsSet() {
		return true
	}

	return false
}

// SetTo gets a reference to the given NullableInt32 and assigns it to the To field.
func (o *WorkItemFilterModelDuration) SetTo(v int32) {
	o.To.Set(&v)
}
// SetToNil sets the value for To to be an explicit nil
func (o *WorkItemFilterModelDuration) SetToNil() {
	o.To.Set(nil)
}

// UnsetTo ensures that no value is present for To, not even an explicit nil
func (o *WorkItemFilterModelDuration) UnsetTo() {
	o.To.Unset()
}

func (o WorkItemFilterModelDuration) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WorkItemFilterModelDuration) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.From.IsSet() {
		toSerialize["from"] = o.From.Get()
	}
	if o.To.IsSet() {
		toSerialize["to"] = o.To.Get()
	}
	return toSerialize, nil
}

type NullableWorkItemFilterModelDuration struct {
	value *WorkItemFilterModelDuration
	isSet bool
}

func (v NullableWorkItemFilterModelDuration) Get() *WorkItemFilterModelDuration {
	return v.value
}

func (v *NullableWorkItemFilterModelDuration) Set(val *WorkItemFilterModelDuration) {
	v.value = val
	v.isSet = true
}

func (v NullableWorkItemFilterModelDuration) IsSet() bool {
	return v.isSet
}

func (v *NullableWorkItemFilterModelDuration) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWorkItemFilterModelDuration(val *WorkItemFilterModelDuration) *NullableWorkItemFilterModelDuration {
	return &NullableWorkItemFilterModelDuration{value: val, isSet: true}
}

func (v NullableWorkItemFilterModelDuration) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWorkItemFilterModelDuration) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


