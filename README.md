...
Note, each API key must be added to a map of `map[string]APIKey` where the key is: Identity.Application and passed in as the auth context for each request.
...
auth := context.WithValue(
		...
		tmsclient.ContextAPIKeys,
		map[string]tmsclient.APIKey{
			"Identity.Application": {Key: "API_KEY_STRING"},
		},
	)
r, err := client.Service.Operation(auth, args)
```
